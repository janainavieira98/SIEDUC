...
@section('page_content')
  @include('components.breadcrumbs', [
    'items' => [
      ...
      __('Edit :entity', ['entity' => __('classroom')]) => url()->current(),
    ]
  ])

  @component('components.card')
    @slot('header')
      <span class="text-capitalize">{{ __('Edit :entity', ['entity' => __('classroom')]) }}</span>
    @endslot
    ...
    <form action="{{ route('classes.update', $classroom) }}" method="post">
            ...
            <select
              ...
              @foreach($periods as $period)
                <option
                  ...
                  value="{{ $period['slug'] }}"
                  {{ old('period_slug', $classroom['period_slug']) == $period['slug'] ? 'selected' : '' }}>
                  {{ $period['description'] }}
                </option>
              @endforeach
            </select>
        ...
        <div class="col-lg-10">
          <div>
            <masked-input
              ...
              id="start_hour"
              type="text"
              value="{{ old('start_hour', $classroom['start_hour']) }}"
        ...
        <div class="col-lg-10">
          <div>
            <masked-input
              ...
              id="end_hour"
              type="text"
              value="{{ old('end_hour', $classroom['end_hour']) }}"
        ...
        <div class="col-lg-10">
          <div>
            <input
              ...
              name="start_date"
              value="{{ old('start_date', $classroom['start_date']) }}"
              required
            />
          </div>
        ...
        <div class="col-lg-10">
          <div>
            <input
              ...
              name="end_date"
              value="{{ old('end_date', $classroom['end_date']) }}"
              required
            />
          </div>
        ...
        <div class="col-lg-10">
          <div>
            <select-multiple
              ...
              :selected="{{ json_encode(old('weekdays', $classroom['weekdays']->pluck('slug'))) }}"
        ...
        <div class="col-lg-10">
          <div>
            <input
              ...
              name="max_users"
              value="{{ old('max_users', $classroom['max_users']) }}"
              required
            />
          </div>
      ...
      <div class="text-right">
        ...
        <button type="submit" class="btn btn-primary text-capitalize">{{ __('save') }}</button>
      </div>
    </form>
  @endcomponent
@endsection
